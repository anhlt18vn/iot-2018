...
public class VIRTUOSO_RESULTS_PROCESS {
  ...
  private void processQueryResult2() {
    String fileName = dataSetName + "_" + deviceName + "_" + Engine + "_QUERY.plot";

    HashMap<Integer, List<Integer>> queryId_result = new HashMap<>();
    ...
    for (File file : listFiles) {
      ...
      queryId_result.put(i++, processQueryLog(file));
    }

    List<String> listString = new ArrayList<>();

    for (int j = 0; j < queryId_result.size(); j++) {
      List<Integer> listInteger = queryId_result.get(j);

      String s = "";

      for (int element : listInteger) {
        s = s + RESULT_PROCESS.numberFormatter.format(element) + "\t";
      }
  ...
  public static void main(String[] args) {
    VIRTUOSO_RESULTS_PROCESS process = new VIRTUOSO_RESULTS_PROCESS("watdiv300", "PII", 10000, 10);
  }
}
